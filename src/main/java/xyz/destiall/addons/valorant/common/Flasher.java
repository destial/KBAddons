...
public interface Flasher {
    void flash(Location source, boolean leftClick);
    ...
    default void flashOut(Player self, Location source) {
        flashOut(self, source, null);
    }

    default void flashOut(Player self, Location source, Entity sourceEntity) {
        ...
        Effects.spawnCrit(source);
        Effects.spawnFlash(source);

        for (LivingEntity livingEntity : entities) {
            if (!selfFlash() && livingEntity.getUniqueId().equals(self.getUniqueId())) {
                continue;
            }
            ...
            RayTraceResult result = source.getWorld().rayTrace(
                    ...
                    true,
                    1d,
                    e -> (e instanceof LivingEntity && e != sourceEntity) || e instanceof BlockDisplay);
